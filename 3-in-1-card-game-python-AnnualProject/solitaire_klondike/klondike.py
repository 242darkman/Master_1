...
class Klondike:
    ...
    def __init__(self, spacing, tableau_spacing):
        ...
        pg.display.set_caption("Solitaire Klondike")
                ...
                if not self.dragged_cards:
                    if self.stock.collidepoint(event.pos):
                        if not self.stock.is_empty():
                            ...
                            cards_from_stock = self.stock.get_3_cards()
                            print("crds from stock --> ", cards_from_stock)
                            for card in cards_from_stock:
                                self.waste.add_card(card)
                                for card_in_waste in self.waste:
                                    card_in_waste.show()  # afficher les cartes piochées dans la pile de stockage
                        else:
